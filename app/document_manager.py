...
class DocumentManager:
    def __init__(self, storage_dir="documents"):
        ...
        # 确保所有文档都是列表格式（向后兼容，处理可能存在的字符串格式）
        for title, content in list(self.documents.items()):
            if isinstance(content, str):
                # 如果是字符串，转换为列表
                self.documents[title] = content.split('\n') if content else []
                print(f"[DocumentManager] 已将文档 '{title}' 从字符串格式转换为列表格式")
    ...
    def _load_documents(self):
        ...
        def contains_default(c):
            s = str(c)
            return ("这是您的试用文档" in s) or ("可以随时添加内容" in s) or ("这是您的默认文档" in s)
