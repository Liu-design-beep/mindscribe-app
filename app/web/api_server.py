...
async def chat(request: ChatRequest):
    ...
    try:
            ...
            if USE_D1 and D1_AVAILABLE and D1_DATABASE_ID:
                try:
                    ...
                    if is_empty_or_old or not is_dev_mode:
                        ...
                        all_documents["通信原理笔记"] = comm_content
                except Exception as e:
                ...
                if is_dev_mode:
                    ...
                    if "介绍文档" not in all_documents or not all_documents.get("介绍文档"):
                        all_documents["介绍文档"] = [
                            ...
                            "",
                            "## ⚠️ 数据存储说明 (当前环境)",
                            "当前系统运行在 **无状态演示模式 (Stateless Demo Mode)**。",
                            "- **未连接 D1 数据库**：系统当前未使用持久化数据库。",
                            "- **临时存储**：所有笔记数据仅保存在服务器内存中。",
                            "- **数据易失性**：当服务器重启或重新部署时，所有新创建的笔记将会丢失。",
                            "- **建议**：请勿在当前模式下存储重要数据。如需持久化存储，请联系管理员配置 D1 数据库。",
                            "",
                            "## 使用说明",
                            "- 对话模式：通过自然语言与系统交互",
                            "- 完整查看：查看所有笔记的完整内容",
                    ...
                    if "通信原理笔记" not in all_documents or not all_documents.get("通信原理笔记"):
                        all_documents["通信原理笔记"] = [
                            ...
                            "### 1.2 通信系统的分类",
                            "",
                            "通信系统可以按照多种方式进行分类。按传输媒介可分为有线通信和无线通信。有线通信使用导线、光纤、同轴电缆等物理媒介，具有传输稳定、抗干扰能力强的优点，但需要铺设线路，成本较高。无线通信使用电磁波在空间中传播，具有灵活性强、覆盖范围广的优点，但容易受到干扰。按信号类型可分为模拟通信和数字通信。模拟通信传输的是连续变化的模拟信号，数字通信传输的是离散的数字信号。数字通信具有抗干扰能力强、易于加密、便于存储和处理等优点，已成为现代通信的主流。",
                            "",
                            "### 1.3 通信系统的主要性能指标",
                            "",
        ...
        if intent == "CONFIRM":
            ...
            default_docs = ["试用文档", "PM问答笔记", "通信原理笔记", "介绍文档", "更新记录日志", "系统提示词"]
            ...
            if intent == "UNKNOWN" or (is_error_content and not is_llm_connection_error):
                ...
                if unknown_count >= 3:
                    ...
                    apology_message = (
                        "灵辑暂时还没能理解你的意思，不过没关系～\n\n"
                        "你的这次反馈已被记录，并将第一时间同步给灵辑团队，我们会尽快进行改进。\n\n"
