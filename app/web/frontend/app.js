    ...
    const updates = [
        {
            ...
            content: '新增智能自动分类写入功能：输入笔记内容后，系统自动语义匹配现有文档并写入；若无匹配文档，左侧弹出绿色确认卡片，由 LLM 生成建议文档名，支持一键新建并写入。同时上线弹窗队列系统，最多同时展示 3 条确认卡片，超出时最旧卡片向上渐出，新卡片从下渐入。'
        },
        {
            date: '2026年3月1日',
            title: '🇨🇳 全面跨越：面向中国用户的交互重构与知识图谱上线',
            content: '深度调研中国用户的使用习惯与认知流程，对界面布局进行系统性重设计，优化操作动线与信息层次；全新推出知识图谱功能，支持将笔记内容自动构建概念关联网络，助力用户直观呼唤存量知识、发现潜在连接。'
        },
        {
            date: '2026年2月26日',
            title: '✨ 交互体验升级：气泡反馈、登录界面重构与删除修复',
            content: '新增每条 AI 回复气泡下方的任务状态反馈栏（✓ 任务已完成 / ✗ 任务失败）与五星评级，方便用户对每次操作给予即时反馈；对登录界面进行全面重新排版，加入像素风格品牌图标与跳动动画，提升首屏识别度；修复删除笔记意图识别失效的问题，删除操作现已正常触发左侧红色确认卡片。'
        },
        {
            date: '2026年1月12日',
            title: '🚀 作品集展示功能上线',
            content: '在开发者模式中新增作品集展示入口，采用黑客终端风格设计，集中展示作者的软硬件设计、机器人控制及算法仿真等多个代表性项目。'
        }
    ];
